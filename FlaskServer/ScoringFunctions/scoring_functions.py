...
def compute_mfcc(x, fs):
    win_length = int(25 * 1e-3 * fs)
    hop_length = int(10 * 1e-3 * fs)
    n_mfcc = 13
    n_mels = 40
    mfcc = librosa.feature.mfcc(y=x, sr=fs, n_mfcc=n_mfcc, n_mels=n_mels, n_fft=win_length, hop_length=hop_length)
    mfcc_norm = (mfcc.T - np.mean(mfcc.T, axis=0)) / (1e-6 + np.std(mfcc.T, axis=0))
    mfcc = mfcc_norm.T
    return mfcc
